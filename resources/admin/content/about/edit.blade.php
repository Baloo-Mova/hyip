    ...
    <div class="row">
        <form method="POST" class="form" enctype="multipart/form-data">
            ...
            <div class="col-md-6">
                ...
                <div class="form-group @if( is_error('image') ) has-error @endif" id="edit-form-image">
            ...
            <div class="col-md-6">
                <div class="form-group @if( is_error('uri') )has-error @endif">
                    <label for="edit-form-uri">* Uri</label>
                    <input type="text"
                           name="uri"
                           value="{{ !empty($old_input['uri']) ? $old_input['uri'] : $item->uri }}"
                           id="edit-form-uri"
                           class="form-control"
                           maxlength="255"
                           required
                           {{ !empty($item->id) ? 'readonly' : '' }}
                    >
                    @if( is_error('uri') )
...
@else
    var deleteItemImage = function() {
        $.getJSON('/admin/about/image-delete/{{ $item->id }}', function(data) {
            if( typeof(data.success) != 'undefined' && data.success == true ) {
                $('#edit-form-image div:first').remove();
                $('#edit-form-image').append(
                    $('<input/>', {type: 'file', name: 'img', id: 'edit-image'})
                );
            }
        });
    };
